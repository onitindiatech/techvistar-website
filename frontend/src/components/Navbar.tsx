          ...
          {activeDropdown === 'services' && (
            ...
            >
              {serviceNavColumns.map((column) => (
                <motion.div key={column.title} variants={columnVariants} className="col-span-3 space-y-3">
                  <div className="mb-2 px-2.5">
                    <div className="text-sm font-extrabold uppercase tracking-wider text-emerald-600 font-display flex items-center gap-1.5">
                      {column.code && <span className="text-emerald-500/80">{column.code}.</span>}
                    ...
                    {column.items.map((srv) => {
                      ...
                      return (
                        <Link
                          ...
                          className="group/item flex items-center gap-2.5 py-1.5 px-2.5 rounded-xl hover:bg-emerald-500/[0.04] transition-all duration-200"
                        >
                          <motion.span whileHover={{ scale: 1.08 }} className="p-1.5 rounded-lg bg-slate-50 text-slate-500 group-hover/item:bg-emerald-100 group-hover/item:text-emerald-700 transition-all duration-200 shrink-0">
                            <IconComp className="w-4 h-4 group-hover/item:scale-110 transition-transform" />
                          </motion.span>
                          <div className="min-w-0 flex-1 flex items-center justify-between">
                            <span className="text-mega font-semibold text-slate-800 group-hover/item:text-emerald-700 transition-colors truncate">{srv.label}</span>
              ...
              {/* Explore All Services Action Link */}
              <div className="col-span-12 pt-4 mt-1 border-t border-slate-100/90 flex items-center justify-between">
                <span className="text-xs font-medium text-slate-500">Looking for our complete list of capabilities?</span>
                <Link
                  to="/services"
                  onClick={() => setActiveDropdown(null)}
                  className="inline-flex items-center gap-1.5 text-sm font-extrabold text-emerald-600 hover:text-emerald-700 transition-colors group/explore cursor-pointer"
                >
        ...
        <AnimatePresence>
          {activeDropdown === 'solutions' && (
            ...
            >
              ...
              {/* Explore All Solutions Action Link */}
              <div className="col-span-12 pt-4 mt-1 border-t border-slate-100/90 flex items-center justify-between">
                <span className="text-xs font-medium text-slate-500">Looking for our complete list of solutions?</span>
                <Link
                  to="/solutions"
                  onClick={() => setActiveDropdown(null)}
                  className="inline-flex items-center gap-1.5 text-sm font-extrabold text-emerald-600 hover:text-emerald-700 transition-colors group/explore cursor-pointer"
                    ...
                    {isMobileServicesOpen && (
                      ...
                      >
                        ...
                        {serviceNavColumns.map((column, columnIndex) => (
                          <div key={column.title} className={cn('space-y-2', columnIndex === 0 ? 'pt-1' : 'pt-3')}>
                            <div>
                              <span className="text-[10px] font-extrabold uppercase tracking-wider text-emerald-600 block">
                                {column.code ? `${column.code}. ` : ''}{column.title}
                              </span>
                              {column.subtitle && (
