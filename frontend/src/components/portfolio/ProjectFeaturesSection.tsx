...
import { Project, ProjectDetailedFeature } from '@/data/projects';
...
export const ProjectFeaturesSection = ({ project }: SectionProps) => {
  const prefersReducedMotion = useReducedMotion();
  const hasDetailed = project.detailedFeatures?.length > 0;
  const hasPlain = project.keyFeatures?.length > 0;

  if (!hasDetailed && !hasPlain) return null;
  ...
  return (
    <section
      id="features"
      className="relative overflow-hidden bg-[#F8FAFC] border border-slate-200/80 rounded-3xl p-6 md:p-10 scroll-mt-24 shadow-sm"
    >
      <div className="absolute inset-0 pointer-events-none z-0 opacity-[0.03]" aria-hidden="true">
        <svg width="100%" height="100%">
          <pattern id="project-features-mesh" width="20" height="20" patternUnits="userSpaceOnUse">
            <path d="M 20 0 L 0 0 0 20" fill="none" stroke="currentColor" strokeWidth="1" />
          </pattern>
          <rect width="100%" height="100%" fill="url(#project-features-mesh)" />
        </svg>
      </div>
      ...
      >
        {hasDetailed
          ? project.detailedFeatures.map((feature: ProjectDetailedFeature, idx: number) => {
              const bgGradient = feature.color || gradientMapping[idx % gradientMapping.length];
              return (
                <motion.div
                  key={idx}
                  ...
                  <div>
                    <div className="flex items-start justify-between gap-4 mb-1 overflow-visible">
                      <div className="icon-btn pointer-events-none scale-50 origin-top-left -mb-8 -mr-4 shrink-0 min-h-[4.5rem] min-w-[4.5rem]">
                        <span className="icon-btn__back" style={{ background: bgGradient }} />
                      ...
                      <div className="flex items-center gap-2">
                        {feature.badge && (
                          <Badge variant="outline" className="text-[9px] font-bold uppercase border-emerald-200 text-emerald-700 bg-emerald-50">
                            {feature.badge}
                          </Badge>
                        )}
                    ...
                    {feature.description && (
                      <p className="text-base text-slate-600 leading-relaxed mt-1">{feature.description}</p>
                    )}
                  </div>
                </motion.div>
              );
