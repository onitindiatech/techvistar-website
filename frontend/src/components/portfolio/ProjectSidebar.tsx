    ...
    >
      <div className="relative space-y-6 overflow-hidden rounded-3xl border-2 border-[#041a3d]/20 bg-white p-6 shadow-sm">
        ...
        <div className="space-y-5">
          {project.client && (
            <div className="flex items-start gap-4 text-xs md:text-sm">
              ...
              <div>
                <p className="text-sm md:text-base font-bold text-slate-900">Client</p>
                <p className="mt-0.5 text-xs md:text-sm text-slate-600 font-medium">{project.client}</p>
              </div>
            </div>
          )}
          ...
          {project.role && (
            <div className="flex items-start gap-4 text-xs md:text-sm">
              ...
              <div>
                <p className="text-sm md:text-base font-bold text-slate-900">Role</p>
                <p className="mt-0.5 text-xs md:text-sm text-slate-600 font-medium">{project.role}</p>
              </div>
            </div>
          )}
          ...
          {formattedDate && (
            <div className="flex items-start gap-4 text-xs md:text-sm">
              ...
              <div>
                <p className="text-sm md:text-base font-bold text-slate-900">Completed</p>
                <p className="mt-0.5 text-xs md:text-sm text-slate-600 font-medium">{formattedDate}</p>
              </div>
            </div>
          )}
          ...
          {matchingIndustries.length > 0 && (
            <div className="flex items-start gap-4 text-xs md:text-sm">
              ...
              <div>
                <p className="text-sm md:text-base font-bold text-slate-900">Industry</p>
                <p className="mt-0.5 text-xs md:text-sm text-slate-600 font-medium">
                  {matchingIndustries.map((ind, idx) => (
                    <span key={ind.id}>
                      {idx > 0 && ', '}
                      <Link to={`/industries/${ind.slug}`} className="font-semibold text-primary hover:underline">
                        {ind.title}
                      </Link>
                    </span>
                  ))}
        ...
        {project.tags?.length > 0 && (
          <div className="space-y-3 border-t border-slate-100 pt-5">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-xs md:text-sm font-bold uppercase tracking-wider text-slate-500">Tags</h3>
            </div>
            <div className="flex flex-wrap gap-1.5">
              {project.tags.map((tag) => (
                <span
                  key={tag}
                  className="rounded-md border border-slate-200 bg-slate-100 px-2.5 py-1 text-xs md:text-sm font-semibold text-slate-600"
                >
                  #{tag}
                </span>
              ))}
        ...
        <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          <Button
            asChild
            className="h-11 w-full rounded-xl bg-[#041a3d] py-2.5 text-sm md:text-base font-extrabold text-white shadow-sm transition-all hover:bg-[#0b2859]"
          >
